...
#文件预测路径
dir_origin_path = "./FlaskProject/uploads/"
dir_save_path = "./FlaskProject/img_save/"
#文件保存路径
path_save = "./FlaskProject/img/"
#预测图片保存路径
path_predict_img = "./FlaskProject/img_save/"

# 批量图像预测保存路径
batch_image_save_path = "./FlaskProject/piliang_img_save/"

#预测视频保存路径
path_predict_video = "./FlaskProject/video_save/"
    ...
    if mode == "video":
        try:
            ...
            while True:
                ...
                try:
                    # 直接使用BGR格式进行处理，避免不必要的颜色空间转换
                    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    frame_pil = Image.fromarray(frame_rgb)
                    
                    # 使用模型进行检测
                    frame_detected = np.array(segformer.detect_image(frame_pil))
            ...
            # 处理缓冲区中剩余的帧
            if frame_buffer and video_save_path != "" and out is not None and out.isOpened():
                for buffered_frame in frame_buffer:
                    out.write(buffered_frame)
                    frames_processed += 1
                frame_buffer.clear()
...
def async_detect(file_id, mode, **kwargs):
    ...
    try:
        ...
        if mode == "image":
            ...
            detection_mode = kwargs.get('detection_mode', 'predict')
            ...
            try:
                ...
                # 创建分割模型实例，启用CUDA
                model = SegFormer_Segmentation(mix_type=0, cuda=cuda_available)
                
                # 使用numpy数组直接创建PIL图像，避免额外的内存复制
                ...
                try:
                    ...
                    result = model.detect_image(
                        image_pil, 
                        count=True if detection_mode != "heatmap" else False,
                        name_classes=['背景', '贴片', '包裹体', '划痕', '其他'],
                    ...
                    if detection_mode == "heatmap":
                        ...
                        try:
                            ...
                            defect_counts = {
                                'patches': int(classes_nums[1]) if len(classes_nums) > 1 else 0,
                                'inclusion': int(classes_nums[2]) if len(classes_nums) > 2 else 0,
                                'scratches': int(classes_nums[3]) if len(classes_nums) > 3 else 0,
                                'other': int(classes_nums[4]) if len(classes_nums) > 4 else 0
                            }
                        ...
                        try:
                            ...
                            print(f"最终类别统计: [背景: {int(classes_nums[0]) if len(classes_nums) > 0 else 0}, 贴片: {int(classes_nums[1]) if len(classes_nums) > 1 else 0}, 包裹体: {int(classes_nums[2]) if len(classes_nums) > 2 else 0}, 划痕: {int(classes_nums[3]) if len(classes_nums) > 3 else 0}, 其他: {int(classes_nums[4]) if len(classes_nums) > 4 else 0}]")
            ...
            # 开始处理视频
            train(video_path=video_path, video_save_path=save_path, video_fps=fps, mode="video")
...
def async_batch_detect(batch_detection_id, files, confidence_threshold='0.6', detection_mode='predict'):
    ...
    参数:
        ...
        detection_mode: 检测模式，可选 'predict'、'confidence'或'heatmap'，默认'predict'
    """
    try:
        ...
        # 初始化检测模型
        segformer = SegFormer_Segmentation(mix_type=0, cuda=torch.cuda.is_available())
        ...
        for idx, file_info in enumerate(files):
            try:
                ...
                # 执行检测
                print(f"使用{detection_mode}模式进行检测")
                result = segformer.detect_image(
                    image, 
                    count=True if detection_mode != "heatmap" else False,
                ...
                if detection_mode == "heatmap":
                    ...
                    result_img = result
                    # 获取类别统计
                    classes_nums = getattr(segformer, 'last_classes_nums', np.zeros(5))
                    defect_counts = {
                        'patches': int(classes_nums[1]) if len(classes_nums) > 1 else 0,
                        'inclusion': int(classes_nums[2]) if len(classes_nums) > 2 else 0,
                        'scratches': int(classes_nums[3]) if len(classes_nums) > 3 else 0,
                    ...
                    if detection_mode == "confidence":
                        # 获取置信度信息
                        confidence_info = getattr(segformer, 'last_confidence_info', {})
                        print(f"置信度检测模式: 阈值={confidence_threshold}")
                        for class_name, info in confidence_info.items():
                            if class_name != '背景' and info['count'] > 0:
                                print(f"  {class_name}: 像素数={info['count']}, 平均置信度={info['avg_confidence']:.4f}")
                ...
                result_path = os.path.join(batch_image_save_path, result_filename)
                ...
                if detection_mode == "confidence":
                    result_info['confidence_info'] = confidence_info
                ...
                if detection_mode == "confidence":
                    individual_metadata['confidence_info'] = confidence_info
...
def backend_camera_detection(output_path, fps=15, detection_id=None, brightness=115, sharpness=1.0):
    ...
    try:
        ...
        # 设置摄像头参数，增加缓冲区大小以提高帧率
        camera_capture.set(cv2.CAP_PROP_BUFFERSIZE, 3)
        ...
        process_interval = 1.0 / (fps / 2)  # 处理频率为设定FPS的一半，但保证更流畅的视频
        ...
        while True:
            ...
            frame_time_elapsed = current_time - last_process_time
            
            ret, frame = camera_capture.read()
            
            if not ret:
                print("无法从摄像头读取帧，跳过")
                time.sleep(0.01)  # 短暂延迟防止CPU占用过高
                continue
            
            frame_count += 1
            
            # 只有当时间间隔大于处理间隔时才处理帧，以维持稳定的帧率
            should_process = frame_time_elapsed >= process_interval
            ...
            if should_process:
                try:
                    ...
                    # 增强清晰度：简化版本，减少处理开销
                    if brightness > 100 and sharpness > 1.0:
                        # 创建更简单的锐化内核
                        kernel_sharpening = np.array([[-0.3, -0.3, -0.3], 
                                                   [-0.3, 2.7, -0.3],
                                                   [-0.3, -0.3, -0.3]])
                    ...
                    # 使用模型处理帧，启用缺陷计数
                    result = model.detect_image(pil_image, count=True, name_classes=['背景', '贴片', '包裹体', '划痕', '其他'])
                    ...
                    # 每5帧更新一次进度和缺陷统计（而不是每10帧）
                    if frames_processed % 5 == 0 and detection_id and detection_id in detection_status:
            ...
            # 不再使用sleep强制控制帧率，而是依靠摄像头本身的帧率和处理间隔
            # 但短暂暂停以避免过度消耗CPU资源
...
def start_camera():
    ...
    try:
        ...
        fps = int(data.get('fps', 20))  # 默认为20FPS（已优化）
...
def get_processed_video(filename):
    ...
    possible_paths = [
        # 标准静态文件路径
        os.path.join(base_dir, 'static', 'processed', 'videos', filename),
        os.path.join(root_dir, 'static', 'processed', 'videos', filename),
        os.path.join('static', 'processed', 'videos', filename),
        # 使用预定义的视频路径
        os.path.join(base_dir, path_predict_video.lstrip('./'), filename),
        os.path.join(root_dir, path_predict_video.lstrip('./'), filename),
        os.path.join(path_predict_video, filename),
        # 检查上传目录
        os.path.join(base_dir, 'uploads', 'videos', filename),
        os.path.join(root_dir, 'uploads', 'videos', filename),
        os.path.join('uploads', 'videos', filename),
        # 检查直接文件名
        filename
    ]
    
    print(f"[get_processed_video] 请求视频文件: {filename}")
    ...
    for path in possible_paths:
        if os.path.exists(path):
            video_path = path
            print(f"[get_processed_video] 在路径中找到视频: {path}")
            break
    
    # 如果没有找到文件
    if not video_path:
        print(f"[get_processed_video] 未找到视频文件 {filename}")
        print(f"[get_processed_video] 尝试过的路径: {possible_paths}")
        return "文件未找到", 404
    
    # 文件存在，返回给客户端
    print(f"[get_processed_video] 返回视频文件: {video_path}")
...
def get_batch_image(filename):
    ...
    try:
        ...
        possible_paths = [
            # 基本目录下的路径
            os.path.join(base_dir, batch_image_save_path.lstrip('./')),
            # 项目根目录下的路径
            os.path.join(root_dir, batch_image_save_path.lstrip('./')),
            # 直接使用定义的批量图像保存路径
            batch_image_save_path,
            # 当前工作目录
            os.path.join(os.getcwd(), batch_image_save_path.lstrip('./'))
        ]
        ...
        for path in possible_paths:
            ...
            if os.path.exists(check_path):
                ...
                found_dir = path
                print(f"[get_batch_image] 在 {path} 中找到文件")
                break
        
        if not file_path:
            # 如果在预定义路径中找不到，尝试在根目录直接查找
        ...
        print(f"[get_batch_image] 批量图像保存路径: {found_dir}")
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    ...
                    if files:
                        ...
                        print(f"[get_batch_image] {path} 目录中可用文件: {files}")
                        break
            
            if not available_files:
                print(f"[get_batch_image] 所有可能的目录中均无可用文件")
            
            return jsonify({'error': '文件不存在', 'checked_paths': possible_paths}), 404
